...
class YoutubePlayerController extends ValueNotifier<YoutubePlayerValue> {
  ...
  /// Plays the video.
  void play() => value.player?.start();

  /// Pauses the video.
  void pause() => value.player?.pause();
  ...
  void load(String videoId, {int startAt = 0, int? endAt}) {
    ...
    if (value.errorCode == 1) {
      ...
      value.player?.setDataSource(loadParams); //FIXME: setDataSource params
    }
  }
  ...
  /// Mutes the player.
  void mute() => value.player?.setVolume(0.0);

  /// Un mutes the player.
  void unMute() => value.player?.setVolume(0.5);

  /// Sets the volume of player.
  /// Max = 100 , Min = 0
  void setVolume(int volume) => volume >= 0 && volume <= 100
      ? value.player?.setVolume(volume / 100)
  ...
  void seekTo(Duration position, {bool allowSeekAhead = true}) {
    value.player?.seekTo(position.inSeconds);
    play();
  ...
  void setSize(Size size) =>
      print('setSize(${size.width}, ${size.height})');
  ...
  void fitHeight(Size screenSize) {
    setSize(screenSize);
    print('setTopMargin("0px")');
  }

  /// Sets the playback speed for the video.
  void setPlaybackRate(double rate) => value.player?.setSpeed(rate);

  /// Toggles the player's full screen mode.
  void toggleFullScreenMode() {
    // TODO: player fullscreen
  ...
  void reload() => value.player?.reset();
